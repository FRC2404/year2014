...
void myRobot::OperatorControl()
{
	...
	while(IsOperatorControl() && IsEnabled())
	{
		...
		float
			...
			x = mylib::setZero( mylib::valSquare( stick1.GetX() ) ),
			y = mylib::setZero( mylib::valSquare( stick1.GetY() ) ),
			z = mylib::setZero( mylib::valSquare( stick1.GetTwist() ) ),
			fl, fr, bl, br;
...
void myRobot::Dashboard()
{
	...
	SmartDashboard::PutNumber("Joystick Axis 1", stick1.GetRawAxis(myRobot::kLeftX));
	SmartDashboard::PutNumber("Joystick Axis 2", stick1.GetRawAxis(myRobot::kLeftY));
